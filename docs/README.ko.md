...
- **DeFi 포지션 관리** - LP, 렌딩, 스테이킹, 볼트 포지션 수동 입력
- **가격 피드** - CoinGecko API를 통한 실시간 가격
- **관심목록** - 실시간 가격 업데이트로 관심 자산 추적
- **리스크 대시보드** - 포트폴리오 리스크 스코어, 레버리지 모니터링, 건전성 지수 추적
- **글로벌 검색** - 빠른 네비게이션 및 자산 검색 (Cmd/Ctrl+K)
